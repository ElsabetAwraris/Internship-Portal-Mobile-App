...
const MyApplicationForCompany = () => {
  const { applications, fetchApplication, accept } =
  ...
  const handleAcceptPress = async (internId) => {
    const result = await accept(internId);
  ...
  const filteredAppplication = applications?.filter((applic) => {
    return applic.applier === userId;
  });
...
export default MyApplicationForCompany;
