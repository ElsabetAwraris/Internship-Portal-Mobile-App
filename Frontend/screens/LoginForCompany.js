...
export default function LoginScreen() {
  ...
  const { login, companyState } = companyAuth();
  const navigation = useNavigation();

  const handleSignIn = async () => {
    const mytoken = await registerForPushNotificationsAsync();
    const result = await login(email, password, mytoken);
  ...
  // companyState.authenticated effect
  useEffect(() => {
    const fetchData = async () => {
      if (companyState.authenticated) {
        navigation.navigate("CompanyDashboardScreen");
      }
    };

    fetchData();
  }, [companyState.authenticated]);

  const handleForgot = () => {
    navigation.navigate("EmailConfirmScreen", { role: "company" });
  };
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        <Image
          ...
          source={{
            uri: "https://media.licdn.com/dms/image/C4D12AQH8HKFuc1B6sg/article-cover_image-shrink_720_1280/0/1632381852847?e=2147483647&v=beta&t=MIQ56mUnRQ3O98DMwcZbYuj9kFw9KJy4ak7vZ5pnLog",
          }}
        />

        <Text style={styles.subtitle}>
          Explore unique opportunities to connect with talented individuals
        </Text>
      </View>
